...
from models.ffb6d_fx import FFB6D
...
def cal_view_pred_pose(model, data, epoch=0, obj_id=-1):
    ...
    with torch.set_grad_enabled(False):
        ...
        # ctr_x, ctr_y, ctr_z = data['ctr_3ds'].squeeze()
        # import pdb;pdb.set_trace()
        
        gt_pose = data['RTs'].squeeze()
        sums = gt_pose.reshape(2, -1).sum(dim=1)
        non_zero_indices = sums.nonzero().squeeze()
        filtered_gt_pose = gt_pose[non_zero_indices]
        list_gt_pose = [filtered_gt_pose.numpy()]
        # gt_RT_np_file = '/home/xietao/fanxuan/FFB6D-master/ffb6d/train_log/ycb/eval_results/gt_RT.npy'
        # gt_RT_np_list = np.load(gt_RT_np_file)
        index = 0
        for cls_id in cu_dt['cls_ids'][0].cpu().numpy():
            ...
            # pose = list_gt_pose[index]
            
            # import pdb; pdb.set_trace()
            # gt_pose = gt_RT_np_list[idx[0]]
            ...
            # gt_mesh_pts = np.dot(mesh_pts, gt_pose[:, :3].T) + gt_pose[:, 3]
            # gt_mesh_p2ds = bs_utils.project_p3d(gt_mesh_pts, 1.0, K)
            # gt_color = bs_utils.get_label_color(obj_id, n_obj=22, mode=2)  
            # np_rgb = bs_utils.draw_p2ds(np_rgb, gt_mesh_p2ds, color=gt_color)
