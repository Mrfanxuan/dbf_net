...
def save_gt_RT_to_txt(gt_RT):
    # 打开文件并写入数据
    with open("/home/xietao/fanxuan/FFB6D-master/ffb6d/train_log/ycb/eval_results/gt_RT.txt", "a") as file:
        # 遍历每个 gt_RT 并将其写入文件
        for item in gt_RT:
            file.write(str(item) + "\n")

def save_gt_RT_to_npy(gt_RT):
    # 打开文件并写入数据
    with open("/home/xietao/fanxuan/FFB6D-master/ffb6d/train_log/ycb/eval_results/gt_RT.npz", "ab") as file:
        # 遍历每个 gt_RT 并将其写入文件
        np.savez(file, gt_RT)
        
def save_poses_txt(pose_list, filename):
...
def iou_3d(box1, box2):
    ...
    # 计算交集的边界点
    min_bounds = np.maximum(corner1_box1, corner1_box2)
    max_bounds = np.minimum(corner2_box1, corner2_box2)
    # import pdb; pdb.set_trace()
    # 计算交集的体积
    intersect_dims = np.maximum(max_bounds - min_bounds, 0)
    ...
    # 计算每个框的体积
    volume_box1 = np.prod(corner2_box1 - corner1_box1)
    volume_box2 = np.prod(corner2_box2 - corner1_box2)
    
    # 计算并集的体积
    union_volume = volume_box1 + volume_box2 - intersect_volume
...
def draw_box3d(ax, point1, point2, color):
    """绘制3D长方体框"""
    # 计算长方体的8个角点
    corners = []
    for x in [point1[0], point2[0]]:
        for y in [point1[1], point2[1]]:
            for z in [point1[2], point2[2]]:
                corners.append([x, y, z])

    # 绘制长方体的边
    for i in range(24):  # 每个长方体有12条边，每条边由2个顶点组成
        p1, p2 = corners[i * 2:(i * 2) + 2]
...
def eval_metric(
    ...
    iou_list = [0]*n_cls
    tt_list = [0]*n_cls
    # iou_list = [list() for i in range(n_cls)]
    gt_RT_np_list = [] # 用于存储gt_RT_np的列表
    
    # gt_RT_np_file = '/home/xietao/fanxuan/FFB6D-master/ffb6d/train_log/ycb/eval_results/gt_RT.npy'
    # try:
    #     gt_RT_np_list = np.load(gt_RT_np_file, allow_pickle=True).tolist()
    ...
    for icls, cls_id in enumerate(cls_ids):
        ...
        gt_RT_np = gt_RT.cpu().numpy()
        gt_RT_np_list.append(gt_RT_np)  # 将gt_RT_np添加到列表中

#————————————————————————————————————————————————————————————————————————————计算IOU TE
        mesh_pts = bs_utils.get_pointxyz_cuda(cls_lst[cls_id-1]).clone()
        xyz = mesh_pts.cpu().numpy()
        pred_rt = pred_RT.cpu().numpy()
        gt_rt = gt_RT.cpu().numpy()
        pred_xyz = np.dot(pred_rt[:, :3], xyz.T) + pred_rt[:, 3][:, np.newaxis]
        gt_xyz = np.dot(gt_rt[:, :3], xyz.T) + gt_rt[:, 3][:, np.newaxis]
        min_x = np.min(gt_xyz[:, 0])
        max_x = np.max(gt_xyz[:, 0])
        min_y = np.min(gt_xyz[:, 1])
        max_y = np.max(gt_xyz[:, 1])
        min_z = np.min(gt_xyz[:, 2])
        max_z = np.max(gt_xyz[:, 2])
        gt_min = np.array([min_x, min_y, min_z]) 
        gt_max = np.array([max_x, max_y, max_z])
        box_gt = (gt_min, gt_max)

        min_x = np.min(pred_xyz[:, 0])
        max_x = np.max(pred_xyz[:, 0])
        min_y = np.min(pred_xyz[:, 1])
        max_y = np.max(pred_xyz[:, 1])
        min_z = np.min(pred_xyz[:, 2])
        max_z = np.max(pred_xyz[:, 2])
        pred_min = np.array([min_x, min_y, min_z]) 
        pred_max = np.array([max_x, max_y, max_z])
        box_pred = (pred_min, pred_max)

        iou = iou_3d(box_pred, box_gt)
        # print(f"The IOU of the two bounding boxes is: {iou}")
        ...
        tt = np.sqrt(((pred_rt[:, 3] - gt_rt[:, 3]) ** 2).sum())  # 计算平方和

        iou_list[cls_id] += iou
        tt_list[cls_id] += tt
    # np.save(gt_RT_np_file, gt_RT_np_list)
    return (cls_add_dis, cls_adds_dis, cls_kp_err, iou_list, tt_list)
...
def eval_one_frame_pose(
    ...
    cls_add_dis, cls_adds_dis, cls_kp_err, iou_list , tt_list = eval_metric(
        cls_ids, pred_pose_lst, pred_cls_ids, RTs, mask, label, gt_kps, gt_ctrs,
        pred_kpc_lst
    )
    return (cls_add_dis, cls_adds_dis, pred_cls_ids, pred_pose_lst, cls_kp_err, iou_list, tt_list)
...
def cal_frame_poses_lm(
    ...
    if cls_msk.sum() < 1:
        ...
        pred_pose_lst.append(pred_RT)
        # save_poses_txt(pred_pose_lst, '/home/xietao/fanxuan/FFB6D-master/ffb6d/train_log/linemod/eval_results/pre_RT/driller_pre_RT.txt')
...
class TorchEval():
    ...
    def __init__(self):
        ...
        self.sym_cls_ids = []
        # self.iou_list_list = [list() for i in range(n_cls)]
        self.iou_list_list = [0]*n_cls
        self.iou_count_list = [0]*n_cls
        self.tt_list_list = [0]*n_cls
    def cal_auc(self):
        add_auc_lst = []
        adds_auc_lst = []
        add_s_auc_lst = []
        ...
        for i in range(len(self.iou_count_list)):
            if i==0:
                continue
            i_iou = self.iou_list_list[i]/self.iou_count_list[i]
            i_tt = self.tt_list_list[i]/self.iou_count_list[i]
            print('------------------>obj{}'.format(i), 'IOU------>', i_iou)
            print('------------------>obj{}'.format(i), 'TE->', i_tt)
    ...
    def eval_pose_parallel(
        ...
        with concurrent.futures.ThreadPoolExecutor(
            ...
            for res in executor.map(eval_func, data_gen):
                if ds == 'ycb':
                    cls_add_dis_lst, cls_adds_dis_lst, pred_cls_ids, pred_poses, pred_kp_errs, iou_list, tt_list = res
                ...
                self.iou_list_list = self.merge_iou_lst(
                    self.iou_list_list, iou_list, self.iou_count_list
                )
                self.tt_list_list = self.merge_tt_lst(
                    self.tt_list_list, tt_list
                )
                
    def merge_iou_lst(self, targ, src, count):
        for i in range(len(targ)):
            targ[i] += src[i]
            if src[i]:
                count[i]+=1
        return targ           
    def merge_tt_lst(self, targ, src):
        for i in range(len(targ)):
            targ[i] += src[i]
        return targ  
